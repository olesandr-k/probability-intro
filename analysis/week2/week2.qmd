...
The wider column are the more inaccurate we see information.
:::
...
sfo_feb_flights %>% 
  ...
  geom_histogram()
sfo_feb_flights %>% 
  TabularManifest::histogram_continuous("arr_delay")
```
...
**The summarise step is telling R to count up how many records of the currently found group are on time - sum(dep_type == "on timeвЂќ) - and divide that result by the total number of elements in the currently found group - n() - to get a proportion, then to store the answer in a new variable called ot_dep_rate.**
...
nycflights %>% 
  mutate(avg_speed = distance/air_time) %>% 
  ggplot(aes(y = avg_speed, x = distance)) + 
  geom_point()
```
...
nycflights %>% 
  mutate(arr_type = ifelse(arr_delay < 5, "on time", "delayed")) %>% group_by(arr_type) %>% 
  summarise(
    c = n()
  ) 
...
```{r}
nycflights %>% print()
ds <- tibble(nycflights)
ds %>% print()
glimpse(ds)
ds %>% 
  group_by(origin, dest, year, month, day, dep_time, tailnum) %>% 
  summarise(
    n = n()
  ) %>% 
  ungroup() %>% 
...
ds %>% 
  group_by(origin, dest, year, month, day, dep_time) %>% 
  mutate(
    n = n()
  ) %>% 
  ungroup() %>% 
  select(origin, dest, year, month, day, dep_time, tailnum, n) %>% 
  filter(
    n > 1
  )

# A grain of dataset is a set of columns that uniquely  define each row 
a <- c(2,5,7,12,13,8)
mean(a)
median(a)
