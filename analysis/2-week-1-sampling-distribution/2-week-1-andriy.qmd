...
# consider formatting the code to be more dev-friendly
```

```{r}
#| code-fold: true
#| code-summary: Store custom summary in a function
# Note that we chose nonstandard evaluations to mimic tidyverse
# But passing variable names as strings may be useful in other contexts
custom_summarize <- function(data, var) {
  data %>%
    summarise(
      ...
      pop_q1 = quantile({{ var }}, 0.25, na.rm = TRUE),  # 25th percentile
      pop_q3 = quantile({{ var }}, 0.75, na.rm = TRUE)   # 75th percentile
    )
}
```
...
```{r}
#| code-fold: true
#| code-summary: Apply custom summary function
ames %>% custom_summarize(area)
ames %>% custom_summarize(price)
```

A good way to display summary of continuous, unimodal variables is a boxplot, which relays similar information as histogram, but also provides some useful point estimates.
...
![Anatomy of a boxplot](https://r-graph-gallery.com/img/other/boxplot_explanation.png)

The boxplot gallery ([link](https://r-graph-gallery.com/boxplot.html)) offer more ideas of using this graph in EDA.
...
## 2. The unknown sampling distribution
...
```{r}
#| code-summary: Modify `custom_summary()` to be more generic

# our previous custom_summarize() baked in the names of the estimates to represent the population. Let's generalize, so we can use either sample or population with this summary function

library(rlang)
custom_summarize <- function(data, var) {
  data %>%
    summarise(
      varname = as_string(ensym(var)), # prints var as string
      ...
      q1 = quantile({{ var }}, 0.25, na.rm = TRUE),  # 25th percentile
      q3 = quantile({{ var }}, 0.75, na.rm = TRUE)   # 75th percentile
    )
}
# how to use
# ds0 %>% custom_summarize(price) # notice NSE for variable

```

```{r samp1-dist}
# type your code for the Exercise here, and Run Document
samp1 %>% custom_summarize(area)
samp1 %>% custom_summarize(price)
```
...
ls_summary <- list(
   ames %>% custom_summarize(area)  %>% mutate(source="population")
  ,ames %>% custom_summarize(price) %>% mutate(source="population")
  ,samp1 %>% custom_summarize(area) %>% mutate(source="sample")
  ,samp1 %>% custom_summarize(price)%>% mutate(source="sample")
)

ls_summary %>% bind_rows() %>% relocate("source")
ds_summary <- ls_summary %>% bind_rows() %>% relocate("source")
```
...
```{r}
# how many percent from pop is in our sample? 
scales::percent(50/nrow(ames),accuracy=.01)
```
...
```{r loop}
sample_means50 <- 
  ames %>% select(area, price) %>% 
  statsr::rep_sample_n( #print the function to examine
    size = 50
    , reps = 15000 # number of groups
    , replace = TRUE
  ) %>% # notice that it returns a grouped dataset
  summarise(x_bar = mean(area))
...
# type your code for the Exercise here, and Run Document
nrow(sample_means50)
sample_means50 %>% custom_summarize(x_bar)
# its sd is the standard error = standard deviation in the distribution of sample means
standar_error <- 
  (ds_summary %>% filter(source=="population",varname=="area") %>% pull(sd))
  /
  sqrt(50)
```

## 3. Interlude: Sampling distributions
...
## 4. Sample size and the sampling distribution
